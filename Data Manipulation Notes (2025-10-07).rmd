...
```{r}
#dataset<- #go to file --> find data --> select--> import
aggressionDat<- read_csv("PSY4802-AggressionSampleData.csv")
```
getting/setting wd
```{r}
...
getwd()
#can change it
#setwd(dir = "~/where you want things to save/")
```
...
#in our dataset, we aren't interested in the demographic data, so we will use the last three columns which are the condition and dependent variables 

#code = pivot_longer(dataset, cols =
#c("columnOfInterest1", "columsOfInterest2" etc), names_to="nameOfVariablesColumn", values_to="nameOfVariablesValuesColumn')

#you can also use indexing to do the column namees you want i.e 2:10
#"names_to" tells R where to put the varibles of interest, while "values_to" tells R what column to put the depend variable values in 

aggressionLongDat<- pivot_longer(aggressionDat, cols =
c("baselineAggression", "postAggression"),names_to="timePoint", values_to="aggressionScore")
```
...
```{r}
relig_income_long<- pivot_longer(relig_income, cols = 2:10, names_to = "income", values_to = "totalParticipants")
```
...
```{r}
#code = gather(dataset, key = "DesiredColumnName", value = "DepenedentVariableValueColumnNames"
#c(varibleOfInterest1, variableOfInterst2, etc))
#you can also use indexing to do the column namees you want i.e 2:10

gather(aggressionDat, key = "timePoint", value = "AggressionScore",
...
```{r}
#weeks have a repeated measure, so it's what we want to put in the long form. Everything not weeks related will stay in short form 
...
#can only use column name once, R won't put that data in more than one place 
```

`pivot_longer(dataframe, columns containing wide data, names_to, values_to)`
3. Convert from wide to long. 
```{r}
ChickWeightLong<- pivot_longer(chickWeightWide, cols = 3:14, names_to = "Time", values_to = "weight")
```
...
```{r}
#using an anxiety score from online (0; <17 mild; 18-24mild/moderate; 25-30 moderate/severe; 30-56 severe)
#code = sample(SampleSize, x = NumbersDrawnFrom, replace = TRUE,) -- you can also include the probablility of each outcome being scored, but we're not doing that here right now
...
```{r}
sampleFrame<- sample(48, x = 0:16, replace = TRUE)
...
```{r}
SampleDat<- data.frame(subjNo=aggressionDat$subjNo, sampleFrame)

practiceDat<- left_join(aggressionDat, SampleDat)
```
...
aggressionDat$parentReportGenderTwo<-
  as.factor((aggressionDat$parentReportGender))
```
...
aggressionDat$parentReportGenderThree<- 
  factor(aggressionDat$parentReportGender, levels = c("F", "M"), labels = c("girl", "boy"))
...
```{r}
aggressionDat %>% select(condition, conditionTwo)
```
...
NewStarwars<- starwars %>% mutate(meters= height/100)
```
