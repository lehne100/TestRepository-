...
```{r}
library(tidyverse) #load after rmisc
data(diamonds)
```
...
diamonds %>% 
  ...
  dplyr::summarise(meanPrice= mean(price), sd(price))
```
...
```{r}
ggplot(diamonds, aes(price))+ geom_histogram(color= "green", fill="bisque") + theme_classic()

ggplot(diamonds, aes(price))+ geom_histogram(color= "black", fill ="#03B5AA")
```
...
```{r}
ggplot(penguins, aes(bill_length_mm))+ geom_histogram(bins=20)
ggplot(penguins, aes(bill_length_mm))+ geom_histogram()

```
2. Include a theme to change the appearance of your graph.
```{r}
ggplot(penguins, aes(bill_length_mm))+ geom_histogram(bins=20)+theme_bw()

```

3. Create a histogram with orange bars and purple lines. 
```{r}
ggplot(penguins, aes(bill_length_mm))+ geom_histogram(bins=20, color="purple", fill="orange")

```

4. **Bonus** Create a histogram with the worst color combination you can think of. 
```{r}
ggplot(penguins, aes(bill_length_mm))+ geom_histogram(bins=20, color = "#F3DE2C", fill = "#001427")

ggplot(penguins, aes(bill_length_mm))+ geom_histogram(bins=20, color="green", fill="chocolate3")

ggplot(penguins, aes(bill_length_mm))+ geom_histogram(bins=20, fill="#22E03B")+theme(panel.background= element_rect(fill = "#2DDD41"))
...
You'll notice that our y axis is "count" and our x axis is "clarity"s. These are not particularly informative. We can change these using xlab and ylab. 
```{r}
...
ggplot(diamonds, aes(clarity))+ 
  geom_bar(color="black", fill="lightgreen")+ 
  theme_classic()+ 
  labs(title= "Graph of Diamond", x= "Diamond clarity", y="frequency")
...
ggplot(diamonds, aes(clarity))+ 
  geom_bar(color="black", fill="lightgreen")+ 
  theme_classic()+ 
  labs(title= "Graph of Diamond", x= "Diamond \nclarity", y="frequency")
...
```{r}
ggplot(priceByColor, aes(color, price))+ geom_bar(stat="identity", fill="lightblue")+ geom_errorbar(aes(ymin= price - se, ymax= price + se))+theme_classic()
...
```{r}
ggplot(priceByColor, aes(color, price))+ geom_bar(stat="identity", fill="lightblue",alpha=.75)+ geom_errorbar(aes(ymin= price - se, ymax= price + se, width=.5 ))+theme_classic()+geom_hline(yintercept = mean(diamonds$price), size= 1, linetype= "dashed", color= "purple")
```

```{r}
ggplot(priceByColor, aes(color, price))+ geom_bar(stat="identity", fill="lightblue",alpha=.75)+ geom_errorbar(aes(ymin= price - se, ymax= price + se, width=.5 ))+theme_classic()+geom_vline(xintercept = mean(diamonds$price), size= 1, linetype= "dashed", color= "purple")
```
Things I changed: 
...
```{r}
ggplot(penguins, aes(species))+geom_bar(fill="red", color="black") +theme_classic()+xlab("species") +ylab("frequency")+ggtitle("penguins")
```
...
```{r}
ggplot(diamonds, aes(color, fill = price))+ geom_bar()+ theme_classic()
```

```{r}
data(mtcars)
ggplot(mtcars, aes(cyl, fill= as.factor(mpg)))+geom_bar()
```
...
```{r}
ggplot(diamonds, aes(carat, price))+geom_point(color="red", fill="lightblue")
#No fill command, "color" is what controls the color you see
```

```{r}
ggplot(diamonds, aes(carat, price))+geom_point(color="lightblue", size = 2, shape= 22)
```
...
```{r}
ggplot(penguins, aes(bill_length_mm, bill_depth_mm))+geom_point(color="#078", size =3, shape = 17)
```
...
```{r}
ggplot(diamonds, aes(carat, price))+geom_point(color="lightblue", size = 2, shape= 22)+ geom_smooth(method="lm", size= 1, color = "grey", linetype="dashed")
```
...
```{r}
ggplot(mtcars, aes(mpg, hp))+geom_point()+ geom_smooth(method = "lm") #use se= FALSE to get rid of condifence interval around the line 
```
...
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point(color="red")
```
3. Change the shape of the points. 
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point(color="red", shape = 11)
```
4. Add a line of best fit to your scatter plot. 
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point(color="red", shape = 11)+ geom_smooth(method ="lm")
```
...
```{r}
ggplot(mentalHealthDat, aes(stressLevel,substanceUse, group = depressionScore, color= depressionScore))+geom_line()
```

```{r}
ggplot(mentalHealthDat, aes(physicalActivity, fill = substanceUse))+ geom_bar(position = position_dodge())+theme_classic()+ xlab("Activity")+ ylab("Frequency")+ ggtitle("Activity and Substance Use")
```
...
```{r}
ggplot(diamonds, aes(price))+geom_boxplot(color="black", fill="lightblue")
```
...
```{r}
mentalHealthDat <- read.csv("~/Downloads/students_mental_health_survey fall 2025.csv")
```
```{r}
