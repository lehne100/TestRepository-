...
```{r}
#make sure you library Rmisc before tidyverse 
library(Rmisc)
```
...
```{r}
#Put the dplyr command just after the pipes if the full table doesn't print out -- my first try I only had one row printing, this solved the problem 
diamonds %>% 
...
data("penguins")
#use na.rm to get rid of the NA. When I first ran the code I didn't have to the na.rm and two penguins reported NA, I had to add na.rm = TRUE so get rid of the NAs
penguinFlipperDat<- summarySE(penguins, measurevar = "flipper_length_mm", groupvars = "species", na.rm = TRUE)
```
